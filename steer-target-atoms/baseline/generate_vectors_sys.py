...
if __name__ == "__main__":
    ...
    for i in range(len(vector_dataset)):
        ...
        if ques and chosen and rejected:
            ...
            print("ques_rej:",ques_rej + rejected)
            ques_chosen_tokens = tokenizer.encode(ques_chosen, return_tensors="pt")
            pos_tokens = tokenizer.encode(ques_chosen + chosen, return_tensors="pt")
            ques_rej_tokens = tokenizer.encode(ques_rej, return_tensors="pt")
            neg_tokens = tokenizer.encode(ques_rej + rejected, return_tensors="pt")
            pos_tokens_list.append(
                {
                    "pos_tokens": pos_tokens.to(device),
                    "ques_tokens_len": ques_chosen_tokens.shape[1],
                    "pos_answer_len": pos_tokens.shape[1] - ques_chosen_tokens.shape[1],
                }
            )
            neg_tokens_list.append(
                {
                    "neg_tokens": neg_tokens.to(device),
                    "ques_tokens_len": ques_rej_tokens.shape[1],
                    "neg_answer_len": neg_tokens.shape[1] - ques_rej_tokens.shape[1],
                }
            )
    ...
    for p_tokens_dict, n_tokens_dict in tqdm(
        ...
        neg_ques_tokens_len = n_tokens_dict["ques_tokens_len"]
        model.reset_all()
        ...
        for layer in args.layers:
            ...
            if args.AB == True:
                ...
                n_activations = n_activations[0, neg_ques_tokens_len:, :].mean(0).detach().cpu()
