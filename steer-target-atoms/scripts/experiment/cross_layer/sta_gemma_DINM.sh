...
for eval_data_name in toxic_DINM_test500 realtoxicity_test500; do
    for ((i=0; i<${layer_num}; i++)); do
        ...
        for ((j=0; j<${trim_num}; j++)); do
            trim=${trims[$j]}
            output_file=./results/${data_name}/${model_name}_results_${mode}/cross_layer/sta/eval_${eval_data_name}/trim${trim}/${model_name}_steer${eval_data_name}_sae_caa_layer${layer}_ef16_${hook_module}_${vector_type}_trim${trim}.result.json
            log_path=./results/${data_name}/${model_name}_results_${mode}/logs/cross_layer/sta/eval_${eval_data_name}/trim${trim}/${model_name}_steer${eval_data_name}_sae_caa_layer${layer}_ef16_${hook_module}_${vector_type}_trim${trim}.result.log
