...
for eval_data_name in toxic_DINM_test500 realtoxicity_test500; do
    ...
    for ((i=0; i<${layer_num}; i++)); do
        layer=${layers[$i]}
        output_file=./results/${data_name}/${model_name}_results_${mode}/cross_layer/sae/eval_${eval_data_name}/${model_name}_steer${data_name}_sae_caa_layer${layer}_16k_${hook_module}_${vector_type}.result.json
        log_path=./results/${data_name}/${model_name}_results_${mode}/logs/cross_layer/sae/eval_${eval_data_name}/${model_name}_steer${data_name}_sae_caa_layer${layer}_16k_${hook_module}_${vector_type}.result.log
